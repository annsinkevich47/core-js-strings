...
function isPalindrome(str) {
  ...
  let inverse = str.split('');
  inverse = inverse.reverse().join('').toString();
  if (inverse === str) {
    return true;
  }
  return false;
...
function findLongestWord(sentence) {
  ...
  for (let i = 0; i < long.length; i++) {
    wordLength.push(long[i].length);
  }
  const position = wordLength.indexOf(Math.max.apply(null, wordLength));
...
function extractNameFromTemplate(value) {
  const notHello = value.slice(7);
  return notHello.replace('!', '');
}
